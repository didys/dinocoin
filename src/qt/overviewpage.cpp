...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeDinocoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDinocoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeDinocoinAmount = strAnonymizeDinocoinAmount.remove(strAnonymizeDinocoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDinocoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeDinocoinAmount * COIN) nMaxToAnonymize = nAnonymizeDinocoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeDinocoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeDinocoinAmount));
        strAnonymizeDinocoinAmount = strAnonymizeDinocoinAmount.remove(strAnonymizeDinocoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDinocoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
