...
std::wstring GetLastErrSzW()
{
    ...
    std::wstring Err = (DNOWSTR)lpMsgBuf;
    LocalFree(lpMsgBuf);
    return Err;
}
...
BOOL Win32RandomAccessFile::_Init( DNOWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
...
BOOL Win32MapFile::_Init( DNOWSTR Path )
{
...
BOOL Win32FileLock::_Init( DNOWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
